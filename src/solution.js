...
var DataTable = {
  ...
  renderTable:  function(data){
    var me =this,
				...
				headerTable = me.createHeaderHTML(data),
				bodyTable = me.createBodyHTML(data);
	...
	createBodyHTML: function(data){
		var me = this,
				...
				headers = me.getHeaders(data);

		for(var i=0, size = data.length; i <size; i++){
			var tr = document.createElement('tr');
			for(var j=0, sizeHeaders = headers.length; j <sizeHeaders; j++){
				var td = document.createElement('td'),
						txtBody = document.createTextNode(value),
				    value = data[i][headers[j]] || '';
	...
	createHeaderHTML: function(data){
		...
		for(var i = 0, size = headers.length; i < size; i++){
			var th = document.createElement('th'),
					txtHeader = document.createTextNode(headers[i]);
			th.appendChild(txtHeader);
			tr.appendChild(th);
		}
		thead.appendChild(tr);
		thead.style.cursor = 'pointer';
		thead.addEventListener('click',me.sortTable	);
		return thead;
	},
